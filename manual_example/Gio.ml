...
module Application_ = struct
  ...
  external signal_connect_activate :
    [> `GApplication ] obj -> (unit -> unit) -> int
    = "ml_Gio_Application_signal_connect_activate"

  external run : [> `GApplication ] obj -> string array -> int
    = "ml_Gio_Application_run"
end
...
class application (self : application_) =
  object
    ...
    method signal_connect_activate callback =
      Application_.signal_connect_activate self callback
